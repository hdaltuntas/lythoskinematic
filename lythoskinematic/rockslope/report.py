...
_METHOD_TR = {
    ...
    "screening": ("Kinematik tarama, Markland (1972) testinin stereografik uygulamasıdır: bir yenilme mekanizması "
                  "ancak kayma yüzeyi ya da kesişim çizgisi şev yüzünde gün ışığına çıkıyor ve eğimi sürtünme "
                  "açısını aşıyorsa mümkündür. Düzlemsel kayma ve kama kayması bu iki koşulla, eğilme-devrilme ise "
                  "Goodman & Bray'in kutup uzayındaki koşuluyla (kutup plunge'ı ≤ ψf − φ) denetlenir. Olasılıksal "
                  "değerlendirmede süreksizlik yönelimlerine normal dağılımlı belirsizlik uygulanır ve Monte Carlo "
                  "simülasyonuyla toplam ve bileşen bazlı yenilme olasılığı bulunur. Kinematik tarama bir güvenlik "
                  "sayısı vermez; hangi mekanizmanın limit denge ile incelenmesi gerektiğini belirler."),
...
_METHOD_EN = {
    ...
    "screening": ("Kinematic screening is the stereographic form of the Markland (1972) test: a failure mechanism "
                  "is possible only where the sliding surface or the line of intersection daylights on the slope "
                  "face and dips more steeply than the friction angle. Planar and wedge sliding are checked against "
                  "those two conditions, and flexural toppling against the Goodman & Bray condition in pole space "
                  "(pole plunge ≤ ψf − φ). For the probabilistic assessment, normally distributed uncertainty is "
                  "applied to the discontinuity orientations and a Monte Carlo simulation gives the overall and "
                  "component-wise probability of failure. Screening yields no factor of safety; it establishes "
                  "which mechanism should be taken to limit equilibrium."),
    ...
    def _header_footer(self, title):
        ...
        def cb(canvas, doc):
            ...
            canvas.drawString(15 * mm, 5.5 * mm,
                              _tr("Lythos Kinematic — kaya şevi analiz aracı",
                                  "Lythos Kinematic — rock slope analysis tool"))
            canvas.restoreState()
        return cb
